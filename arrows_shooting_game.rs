// Part of the Course: https://thameswater.udemy.com/course/ultimate-rust-crash-course/learn/lecture/19621272#overview
...
fn main() {
    ...
    for coord in arrow_coords{
        ...
        let shot_value  = match coord.distance_from_center() {
            x if x < 1.0 => Shot::Bullseye, //Everything under 1 bullseye
            x if x <= 5.0 => Shot::Hit(x), // This considers from 1 to 5 (including)
            _ => Shot::Miss, // "_" wildcard any other cases do miss
        };
        shots.push(shot_value); //Finally append the shot value  (enum ) into the Shots vec
    }
//   for coord in arrow_coords.iter() {
//       coord.print_description();
//       let shot_value : f64 = coord.distance_from_center();
//       if shot_value < 1.0 {shots.push(Shot::Bullseye)}
//       else if shot_value >= 1.0 && shot_value  <= 5.0 {shots.push(Shot::Hit(shot_value))}
